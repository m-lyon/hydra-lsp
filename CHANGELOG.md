...
- Moved analysis off the async runtime onto two `rayon` pools (a small latency pool for interactive requests, a worker pool for diagnostics), sized by the new `numThreads` initialization option
